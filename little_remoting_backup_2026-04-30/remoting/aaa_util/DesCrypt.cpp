...
void DesCrypt::encrypt(unsigned char *dst, const unsigned char *src, size_t dataLen,
...
void DesCrypt::decrypt(unsigned char *dst, const unsigned char *src, size_t dataLen,
                       const unsigned char *key)
{
  deskey(key, DECRYPT);
  for (size_t i = 0; i < dataLen; i += 8) {
...
void DesCrypt::deskey(const unsigned char hexKey[8], OperationMode mode)
{
  int i, j, l, m, n;
