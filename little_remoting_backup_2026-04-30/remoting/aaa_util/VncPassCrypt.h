...
public:
  ...
  void updatePlain(const ::u8 cryptedPass[::subsystem::VncPassCrypt::VNC_PASSWORD_SIZE]);

  // Encrypt the plain password and fills by encrypted values the
  // encryptedPass[8] array.
  static void getEncryptedPass(::u8 encryptedPass[::subsystem::VncPassCrypt::VNC_PASSWORD_SIZE],
  ...
  static void getPlainPass(::u8 plainPassword[::subsystem::VncPassCrypt::VNC_PASSWORD_SIZE],
                                         const ::u8 encryptedPass[::subsystem::VncPassCrypt::VNC_PASSWORD_SIZE]);

  // Returns true if it's matched.
  bool challengeAndResponseIsValid(const ::u8 challenge[16],
