 ...
 * These routines provide selection of a custom color ::std::map for an image,
 * followed by mapping of the image to that color ::std::map, with optional
 * Floyd-Steinberg dithering.
 * It is also possible to use just the second pass to ::std::map to an arbitrary
 * externally-given color ::std::map.
 *
 * Note: ordered dithering is not supported, since there isn't any fast
 * way to compute intercolor distances; it's unclear that ordered dither's
 * fundamental assumptions even hold with an irregularly spaced color ::std::map.
 */
 ...
 *
 * We re-use the histogram space as an "inverse color ::std::map", essentially a
...
start_pass_2_quant (j_decompress_ptr cinfo, boolean is_pre_scan)
{
  ...
  /* Zero the histogram or inverse color ::std::map, if necessary */
...
new_color_map_2_quant (j_decompress_ptr cinfo)
{
  ...
  /* Reset the inverse color ::std::map */
  cquantize->needs_zeroed = TRUE;
}
