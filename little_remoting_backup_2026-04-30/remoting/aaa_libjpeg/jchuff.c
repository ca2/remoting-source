...
typedef struct {
  ::u32 ehufco[256];	/* code for each symbol */
...
typedef struct {
  ...
  ::u32 EOBRUN;		/* run length of EOBs */
  ::u32 BE;		/* # of buffered correction bits before MCU */
...
INLINE
LOCAL(boolean)
emit_bits_s (working_state * state, ::u32 code, int size)
/* Emit some bits; return TRUE if successful, FALSE if must suspend */
{
  /* This routine is heavily used, so it's worth coding tightly. */
  register int put_buffer;
  ...
  put_buffer = ((int) code) & ((((int) 1) << size) - 1);
...
INLINE
LOCAL(void)
emit_bits_e (huff_entropy_ptr entropy, ::u32 code, int size)
		    ...
		    ::u32 nbits)
{
  ...
  while (nbits > 0) {
    emit_bits_e(entropy, (::u32) (*bufstart), 1);
    bufstart++;
    nbits--;
  }
}
...
encode_mcu_DC_first (j_compress_ptr cinfo, JBLOCKARRAY MCU_data)
{
  ...
  for (blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
    ...
    if (nbits)			/* emit_bits rejects calls with size 0 */
      emit_bits_e(entropy, (::u32) temp2, nbits);
  }
...
encode_mcu_DC_refine (j_compress_ptr cinfo, JBLOCKARRAY MCU_data)
{
  ...
  for (blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
    ...
    emit_bits_e(entropy, (::u32) (MCU_data[blkn][0][0] >> Al), 1);
  }
...
encode_mcu_AC_refine (j_compress_ptr cinfo, JBLOCKARRAY MCU_data)
{
  ...
  for (k = cinfo->Ss; k <= Se; k++) {
    ...
    emit_bits_e(entropy, (::u32) temp, 1);
		  ...
		  c_derived_tbl *dctbl, c_derived_tbl *actbl)
{
  ...
  /* or the complement of its magnitude, if negative. */
  if (nbits)			/* emit_bits rejects calls with size 0 */
    if (! emit_bits_s(state, (::u32) temp2, nbits))
      return FALSE;
...
#define MAX_CLEN 32		/* assumed maximum initial code length */
  unsigned char bits[MAX_CLEN+1];	/* bits[k] = # of symbols with code length k */
