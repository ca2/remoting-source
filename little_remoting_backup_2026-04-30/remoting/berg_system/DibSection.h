...
public:
  ...
  // the DIB section.
  DibSection(const ::innate_subsystem::PixelFormat & pixelformat, const ::i32_size & size, HWND compatibleWin = 0);
  virtual ~DibSection();
...
private:
  ...
  void openDIBSection(const ::innate_subsystem::PixelFormat & pixelformat, const ::i32_size & size, HWND compatibleWin);
  void closeDIBSection();
  void releaseTargetDC();

  // This function throwing an exception on a failure.
  void blitToDibSection(const ::i32_rectangle &  rectangle, DWORD flags);

  // This function throwing an exception on a failure.
  void blitFromDibSection(const ::i32_rectangle &  rectangle, DWORD flags);
  void stretchFromDibSection(const ::i32_rectangle &  rectangleSource,const ::i32_rectangle & rectangleTarget, DWORD flags);

  void setupBMIStruct(BITMAPINFO *pBmi, const ::innate_subsystem::PixelFormat & pixelformat, const ::i32_size & size);
