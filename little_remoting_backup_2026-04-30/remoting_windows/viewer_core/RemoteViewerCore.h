...
namespace remoting
{
   ...
   //
   class CLASS_DECL_REMOTING RemoteViewerCore :
   ...
   public:
      ...
      //
      // To allow logging, pass a pointer to an object which implements the ::subsystem::LogWriter
      // interface (see log_writer/LogWriter.h). Logging is disabled by default.
      //
      RemoteViewerCore(::subsystem::LogWriter * plogwriter = 0);
      ...
      //
      // Request full refresh of the pframebuffer, so that the whole screen will be
      ...
      // the protocol (when the pframebuffer does not exist yet).
      //
      ...
      ::i32_rectangle_array_base getDesktops();
      ...
      Lockable < lockable_critical_section > m_criticalsectionFramebuffer;
      ::innate_subsystem::Framebuffer m_pframebuffer;

      // ::list_base of server dispalys
      ::i32_rectangle_array_base m_desktops;
      ...
      ::innate_subsystem::Framebuffer m_pframebufferRectangle;
