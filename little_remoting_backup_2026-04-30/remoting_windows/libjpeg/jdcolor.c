...
typedef struct {
  ...
  int * rgb_y_tab;		/* => table for RGB to Y conversion */
...
/* Normal case, sYCC */
{
  ...
  cconvert->Cb_b_tab = (int *) (*cinfo->mem->alloc_small)
    ((j_common_ptr) cinfo, JPOOL_IMAGE, (MAXJSAMPLE+1) * SIZEOF(int));
  cconvert->Cr_g_tab = (int *) (*cinfo->mem->alloc_small)
    ((j_common_ptr) cinfo, JPOOL_IMAGE, (MAXJSAMPLE+1) * SIZEOF(int));
  cconvert->Cb_g_tab = (int *) (*cinfo->mem->alloc_small)
    ((j_common_ptr) cinfo, JPOOL_IMAGE, (MAXJSAMPLE+1) * SIZEOF(int));
		 ...
		 JSAMPARRAY output_buf, int num_rows)
{
  ...
  register int * Crgtab = cconvert->Cr_g_tab;
  register int * Cbgtab = cconvert->Cb_g_tab;
  SHIFT_TEMPS
...
build_rgb_y_table (j_decompress_ptr cinfo)
{
  ...
  cconvert->rgb_y_tab = rgb_y_tab = (int *) (*cinfo->mem->alloc_small)
    ((j_common_ptr) cinfo, JPOOL_IMAGE, TABLE_SIZE * SIZEOF(int));
		  ...
		  JSAMPARRAY output_buf, int num_rows)
{
  my_cconvert_ptr cconvert = (my_cconvert_ptr) cinfo->cconvert;
  register int r, g, b;
  register int * ctab = cconvert->rgb_y_tab;
  register JSAMPROW outptr;
  register JSAMPROW inptr0, inptr1, inptr2;
  register JDIMENSION col;
  JDIMENSION num_cols = cinfo->output_width;
