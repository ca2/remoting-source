 ...
 * These routines provide mapping to a fixed color ::std::map using equally spaced
 ...
 * image.  We use a ::std::map consisting of all combinations of Ncolors[i] color
...
typedef struct {
  ...
  JSAMPARRAY sv_colormap;	/* The color ::std::map as a 2-D pixel array */
...
/* Return largest input value that should ::std::map to j'th output value */
...
create_colormap (j_decompress_ptr cinfo)
{
  ...
  /* The colors are ordered in the ::std::map in standard row-major order, */
...
make_odither_array (j_decompress_ptr cinfo, int ncolors)
{
   ...
   */
  den = 2 * ODITHER_CELLS * ((int) (ncolors - 1));
  for (j = 0; j < ODITHER_SIZE; j++) {
    for (k = 0; k < ODITHER_SIZE; k++) {
      num = ((int) (ODITHER_CELLS-1 - 2*((int)base_dither_matrix[j][k])))
	    * MAXJSAMPLE;
