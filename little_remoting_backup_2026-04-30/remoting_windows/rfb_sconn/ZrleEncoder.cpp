...
namespace remoting
{
                              ...
                              const EncodeOptions *options)
   {
      ...
      for (tileRect.top = rectangle.top; tileRect.top < rectangle.bottom; tileRect.top += TILE_SIZE) {
         ...
         tileRect.bottom = min(rectangle.bottom, tileRect.top + TILE_SIZE);

         for (tileRect.left = rectangle.left; tileRect.left < rectangle.right; tileRect.left += TILE_SIZE) {

            tileRect.right = min(rectangle.right, tileRect.left + TILE_SIZE);
      ...
      if (m_rgbData.empty()) {
         ...
         m_deflater.setInput(reinterpret_cast<const_char_pointer >(&m_rgbData.front()),
                             m_rgbData.size());
         m_deflater.deflate();
