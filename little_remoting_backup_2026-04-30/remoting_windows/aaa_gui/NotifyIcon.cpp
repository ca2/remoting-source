...
namespace remoting
{
   ...
   void NotifyIcon::setText(const ::scoped_string & scopedstrText)
   {
      ...
      _tcsncpy_s(m_nid.szTip, BUFFER_SIZE_TCHARS,::wstring(scopedstrText).c_str(), _TRUNCATE);
                         ...
                         DWORD timeoutMillis)
   {
      ...
      _tcsncpy_s(m_nid.szInfo, 255, ::wstring(scopedstrMessage).c_str(), _TRUNCATE);
      _tcsncpy_s(m_nid.szInfoTitle, 63, ::wstring(scopedstrCaption), _TRUNCATE);
