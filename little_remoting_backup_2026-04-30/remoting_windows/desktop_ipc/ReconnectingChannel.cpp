...
namespace remoting
{
   ...
   void ReconnectingChannel::initialize_reconnecting_channel(::u32 timeOut, ::subsystem::LogWriter * plogwriter)
   {
   ...
   void ReconnectingChannel::waitForReconnect(const ::scoped_string &scopedstrFunName, Channel *channel)
   {
      ...
      while (!success)
      {
         ::u32 timeForWait = maximum((m_iTimeout - timeStart.elapsed()).integral_millisecond(), 0);
